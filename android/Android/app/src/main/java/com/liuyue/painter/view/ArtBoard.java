...
import com.liuyue.painter.shape.Circle;
import com.liuyue.painter.shape.Gallery;
import com.liuyue.painter.shape.Ink;
import com.liuyue.painter.shape.Line;
import com.liuyue.painter.shape.Point;
import com.liuyue.painter.shape.Rectangle;
import com.liuyue.painter.shape.Shape;
import com.liuyue.painter.utils.XmlOperation;
...
//                }
    ...
    public void undo() {
        if (mSaveShapeList != null && mSaveShapeList.size() >= 1) {
            mDeleteShapeList.add(mSaveShapeList.get(mSaveShapeList.size() - 1));
            mSaveShapeList.remove(mSaveShapeList.size() - 1);
            try {
                // 重新绘制图案
                redrawOnBitmap();
            } catch (Exception e) {
    ...
    public void redo() {
        if (mDeleteShapeList != null && mDeleteShapeList.size() >= 1) {
            mSaveShapeList.add(mDeleteShapeList.get(mDeleteShapeList.size() - 1));
            mDeleteShapeList.remove(mDeleteShapeList.size() - 1);
            try {
                // 重新绘制图案
                redrawOnBitmap();
            } catch (Exception e) {
