...
//                }
    ...
    private boolean IsNotInside(float x, float y) {
        if (NeedMoveRect != null) {
            return x >= NeedMoveRect.left && x <= NeedMoveRect.right && y >= NeedMoveRect.bottom && y <= NeedMoveRect.top;
        }
        return false;
    }
...
//        System.out.println("EraserState:"+EraserState);
        ...
        if (scanFilePath.isDirectory()) {
            for (File file : scanFilePath.listFiles()) {
                ...
                if (fileAbsolutePath.endsWith(".xml")) {
                    // 将xml解析放入mGallery中
                    try {
                        mGallery.AddPainting(XmlOperation.TransXmlToShape(fileAbsolutePath),
                                mBitmap);
    ...
    public boolean isEdited() {
        return PreShapeListSize != SaveShapeList.size();
    }
